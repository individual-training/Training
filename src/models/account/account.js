...
export default  {
  ...
  effects: {
    ...
    *GetUserTraining({ payload, success }, { select, call, put }) {
      ...
      const result = yield call(getUserTraining,{part_id:partId, carid:cardId, day:day });
      if(result){
    ...
    *PostUserTraining({ payload, success }, { select, call, put }) {
      ...
      const result = yield call(postUserTraining,{part_id:partId, carid:cardId, date:search });
      if(result){
        yield put({type:'setData',payload:result});
      }
      yield put({type:'setLoading',payload:false})
