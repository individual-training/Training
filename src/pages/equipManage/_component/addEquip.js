...
@Form.create({
  mapPropsToFields(props) {
    ...
    Object.keys(data).forEach((key) => {
      fields[key] = Form.createFormField({ value: `${data[key] ===0 ? 0 : data[key] || ''}` })
    })
    return fields
  },
...
export default class AddEquipComp extends React.Component {
  ...
  reset = () => {
    const { dispatch, editEquip:{dataStringify}} = this.props;
    dispatch({type:'editEquip/setEquip',payload:JSON.parse(dataStringify)})
  }
    ...
    return (
      ...
      >
        ...
        <div style={{padding:'10px 120px'}} className={styles.flexAround}>
          <Button className={`${styles.width110} ${styles.greyBtn}`} onClick={this.reset} >重置</Button>
          <Button className={`${styles.width110} ${styles.greenBtn}`} onClick={this.submit}>确定</Button>
        </div>
      </Modal>
    );
  }
