...
export default class UserPage extends React.Component{
  ...
  getColumns = () => {
    const { listInfo:{ department } } = this.props.userManageList;
    return [
      {
        title: '人员姓名',
        dataIndex: 'uname',
      ...
      },
      {
        ...
        render:(text,record)=>{
          return(
            <div>
              <a className={styles.operate} onClick={()=>{this.editUser(record)}}>
                <img src={require('../../static/icon/edit.png')} alt=""/>
                <span>编辑</span>
              </a>
              <a className={styles.operate} onClick={() => {this.deleteUser(record)}}>
    ...
    return (
      <React.Fragment>
        <div className={styles.search}>
          <Row gutter={40}>
            <Col span={2} style={{textAlign:'right'}} ><span className={styles.title}>姓名</span></Col>
            <Col span={5}>
              <Input placeholder={'请输入姓名'} onChange={this.nameChange} value={searchName}></Input>
            </Col>
            <Col span={3} style={{textAlign:'right'}} ><span className={styles.title} >所属部门</span></Col>
            <Col span={5}>
              <Select
            ...
            <Col span={4} style={{textAlign:'right'}}>
              <Button className={`${styles.width110} ${styles.greenBtn}`} onClick={this.search}>搜索</Button>
            </Col>
            <Col span={4} style={{textAlign:'right'}}>
              <Button className={`${styles.width110} ${styles.greenBtn}`}  onClick={this.toAddUser} >添加人员</Button>
            </Col>
          </Row>
        </div>
        <Table dataSource={this.getDataSource()} columns={this.getColumns()} loading={ loading } pagination={pagination}/>
        <EditUser></EditUser>
      </React.Fragment>
    )
  }
